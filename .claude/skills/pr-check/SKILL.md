...
If the overall result is **READY**, automatically generate a PR description in markdown. Analyze all commits on the branch (`git log main..HEAD`) and the actual code changes (`git diff main..HEAD`) to produce:
...
Present the generated PR message to the user in a code block so they can copy it directly. Do not include the fact that it was generated by claude, just stay to the point and summarize accurately.
