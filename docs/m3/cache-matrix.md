<!-- Loaded by: skills/cache-audit, skills/pr-check, skills/m3-orchestrator -->

# M3 Cache Matrix

Source of truth for caching: M2 §4.4. **All M2 caching invariants carry over to M3 unchanged** per `docs/m3/uber-m3.md` line 43 — "Redis caching (all cached endpoints remain cached)". The only structural change is database isolation (per-service PG); caches still live in the shared Redis instance, keyed per service.
...
**Wildcard invalidation:** `redis-cli SCAN + DEL` (or `KEYS + UNLINK` for small caches). Over-invalidation is acceptable.
...
**Per-feature TTLs:** F1=5m (search), F3=10m (DTO), F5=5m (JSONB), F6=10m (report), F8=15m (relationship), F9=10m (combined).
...
## Invalidation — M2 Carry-Over Rules
...
- Caching is **intra-service** — each service's cache writes/reads use its own service prefix. Cross-service invalidation does not cross PG boundaries; it crosses Redis key prefixes.
- A RabbitMQ consumer that mutates local state must invalidate the matching local-service cache after the mutation succeeds, identical to a direct M1/M2 write. (E.g., `ride.completed` consumer in driver-service flips driver status to AVAILABLE → invalidate `driver-service::driver::{driverId}`.)
- New M3 endpoints exposed for saga pre-checks (e.g., `GET /api/locations/driver/{driverId}/recent` per uber-m3.md:1228) are **not** cached — they need fresh data per call.
