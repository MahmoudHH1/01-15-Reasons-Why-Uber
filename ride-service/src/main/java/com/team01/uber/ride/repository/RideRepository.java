...
public interface RideRepository extends JpaRepository<Ride, Long> {
    ...
    // S3-F3: surge pricing — counts active rides near a location (local rides table only)
    ...
    // Active = REQUESTED, ACCEPTED, IN_PROGRESS, COMPLETED, PAYMENT_PENDING (per §5)
    @Query("SELECT COUNT(r) FROM Ride r WHERE r.userId = :userId AND r.status IN (:statuses)")
    ...
    // GET /api/rides/driver/{driverId}/summary — used by S2-F3, S2-F12
    @Query("SELECT COUNT(r) FROM Ride r WHERE r.driverId = :driverId AND r.status IN (:statuses)" +
            " AND (:start IS NULL OR r.requestedAt >= :start)" +
            " AND (:end IS NULL OR r.requestedAt < :end)")
    ...
    // setDriverAvailable()                    → ride.completed/cancelled event consumed by driver-service
    ...
    // findDriverVehicleTypeById()             → DriverServiceClient.getDriver().vehicleDetails()
    // getTotalRevenueForCompletedRidesFromPayments() → local rides.fare used instead (§5 S3-F10 note)
}